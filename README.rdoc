...
Builder for creating tables inspired by ActionView::Helpers::FormHelper::FormBuilder.
...
You can pass blocks to the d and h methods:

  <%= t.d(:class => 'name') do %>
    <%= link_to(h(front_man.name), front_man_url) %>
  <% end %>

All tags can have extra html options.
